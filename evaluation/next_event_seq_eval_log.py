...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Next-event evaluation (measure-mode): sample context windows and predict the following tokens (NEXT-N) via iterative MLM masks."
    )
    ...
    # Toggle whether we "leak" the true target event-type into the generated MASK positions
    p.add_argument(
    ...
    p.add_argument("--horizon", type=int, default=10, help="Max how many next tokens to predict after the sampled context.")
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Output CSV path for per-step (predicted, true) pairs.",
    )
...
def _build_eval_example_nextn(
    ...
    *,
    ...
    - Targets = up to `horizon` tokens starting at that position, but clipped to sequence end (H_eff may be < horizon).
    """
    L = int(max_len)
    H = int(horizon)
    if H < 1:
    ...
    # Capacity: we keep one slot free (ctx_cap = L-1) for iterative append/mask positions
    ctx_cap = L - 1
    ...
    # Random prediction start in rest
    pred_start = random.randint(0, len(rest) - 1)

    # Context immediately before pred_start (up to rest_ctx_cap tokens)
    ctx_left = max(0, pred_start - rest_ctx_cap)
    ctx_rest = rest[ctx_left:pred_start]

    # Targets from pred_start forward, clipped to end
...
def evaluate_next_event_nextn_token(
    ...
    *,
    ...
    horizon: int,
    pairs_csv_path: str,
    token_id_to_vocab: Dict[int, int],
    ...
    sum_pairwise_acc_at_k = {k: 0.0 for k in topk}  # mean over patients, using valid steps only
    ...
    try:
        for pi, rec in enumerate(tqdm(records, desc="patients")):
            ...
            valid_steps_patient = 0  # steps that are actually evaluated (i.e., not skipped due to true_vocab=-1)
            ...
            for step in range(H_eff):
                ...
                pred_vocab = int(token_id_to_vocab.get(int(pred_top1_tok), -1))

                # Write one row per (non-skipped) step
                writer.writerow([pid_str, int(step), int(pred_top1_tok), int(true_tok), pred_vocab, int(true_vocab)])
            ...
            # Only count patient if at least 1 valid step contributed
            ...
            print(f"--- patient={pi} horizon_used={H_eff}/{H_max} count_score (TOKEN-level) ---")
            for k in topk:
    ...
    # mean over patients (only those with >=1 valid step)
    for k in topk:
...
def main() -> None:
    ...
    if args.pairs_csv is None:
        ...
        args.pairs_csv = os.path.join("..", "out", "evaluation", f"next_event_seq_pairs_{ckpt_base}.csv")
