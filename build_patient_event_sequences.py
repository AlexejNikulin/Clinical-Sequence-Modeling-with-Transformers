...
def build_stage_sequence_with_counts(df: pd.DataFrame, subject_id: int):
    """
    ...
    for _, row in df.iterrows():
        stage = SOURCE_TO_STAGE.get(row["source"])
        if stage is None:
            continue
...
def build_transition_graph(stage_sequence):
    G = nx.DiGraph()
...
def draw_stage_graph(G, output_file="stage_graph.png"):
    """
...
def visualize_sequence(df, subject_id):
    """
    ...
    stage_sequence = build_stage_sequence_with_counts(df, subject_id)
    G = build_transition_graph(stage_sequence)
