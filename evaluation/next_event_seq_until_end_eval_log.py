...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Next-event evaluation (measure-mode): sample context windows and predict following tokens (NEXT-N) via "
            "iterative MLM mask with early stopping on stop-vocab. Includes CSV logging + event-type leak toggle + "
            "true_vocab==-1 skips + length-independent sampling."
        )
    )
    ...
    p.add_argument(
        ...
        type=str,
        default="12,13",
        help="Comma-separated list of stop vocabulary/group IDs. Generation stops when pred OR true hits one of these (inclusive).",
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "event type for that step (strong hint). If False, do NOT leak target event type; instead, use last attended "
            "event type or default_event_type_id."
        ),
    )
    ...
    *,
    ...
    try:
        for pi, rec in enumerate(tqdm(records, desc="patients")):
            ...
            # Metrics bookkeeping (only over VALID evaluated steps)
            ...
            for step in range(min(Hmax, len(true_next_tokens))):
                ...
                # update true_count for this VALID step (token-level bag metric)
                true_count[true_tok] = true_count.get(true_tok, 0) + 1

                # pairwise acc@k (TOKEN-level) + count bookkeeping
                for k in topk:
                ...
                # commit predicted token into context for next step
                xg[gen_pos] = pred_top1_tok

                # --- early stop conditions (inclusive, evaluated after this step) ---
                stopped_by = ""
...
def main() -> None:
    ...
    if args.pairs_csv is None:
        ...
        args.pairs_csv = os.path.join("..", "out", "evaluation", f"next_event_token_until_end_pairs_{ckpt_base}.csv")
